...
class MyAugmentedMnist(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    data_path = pathlib.Path(__file__).parent.absolute() / "data/augmented"
    return [
        tfds.core.SplitGenerator(
            name=tfds.Split.TRAIN,
            gen_kwargs=dict(
                num_examples=60_000,
                data_path=f"{data_path}/aug_train_data.npy",
                label_path=f"{data_path}/aug_train_labels.npy",
            )),
        tfds.core.SplitGenerator(
            name=tfds.Split.TEST,
            gen_kwargs=dict(
                num_examples=10_000,
                data_path=f"{data_path}/aug_test_data.npy",
                label_path=f"{data_path}/aug_test_labels.npy",
            )),
    ]
...
def _extract_mnist_images(image_filepath, num_images):
  return np.load(image_filepath)

def _extract_mnist_labels(labels_filepath, num_labels):
  return np.load(labels_filepath)
